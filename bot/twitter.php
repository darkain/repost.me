...
class twitter{
	 ...
	 * @var array
	 */
	var $headers=array('Expect:', 'X-Twitter-Client: 1.0','X-Twitter-Client-Version: 1.0','X-Twitter-Client-URL: ');
	...
	function update( $status, $replying_to = false )
	{
        ...
        $request = 'http://api.twitter.com/1/statuses/update.' . $this->type;
	...
	/**
	 * ReTweet another user's status update
	 * @param long int $id exist status update's ID number
	 * @return string|boolean
	 */
    function retweet( $id )
    {
        ...
        $request = 'http://api.twitter.com/1/statuses/retweet/' . $id . '.' . $this->type;
	...
	function getReplies( $page = false, $since = false, $since_id = false )
	{
	    ...
	    $request = 'http://api.twitter.com/1/statuses/replies.' . $this->type . $qs;    
	...
	function deleteStatus( $id )
    {
        ...
        $request = 'http://api.twitter.com/1/statuses/destroy/' . (int) $id . '.' . $this->type;
	...
	function publicTimeline( $sinceid = false )
	{
        ...
        $request = 'http://api.twitter.com/1/statuses/public_timeline.' . $this->type . $qs;
	...
	function userTimeline($id=false,$count=20,$since=false,$since_id=false,$page=false)
	{
        ...
        if( $id === false )
            $request = 'http://api.twitter.com/1/statuses/user_timeline.' . $this->type . $qs;
        else
            $request = 'http://api.twitter.com/1/statuses/user_timeline/' . rawurlencode($id) . '.' . $this->type . $qs;
	...
	function showStatus( $id )
	{
        ...
        $request = 'http://api.twitter.com/1/statuses/show/'.intval($id) . '.' . $this->type;
		return $this->objectify( $this->process($request) );
    }
	...
	function friends( $id = false, $page = false )
	{
	    ...
	    $request = ( $id ) ? 'http://api.twitter.com/1/statuses/friends/' . $id . '.' . $this->type . $qs : 'http://api.twitter.com/1/statuses/friends.' . $this->type . $qs;
		return $this->objectify( $this->process($request) );
	}
	...
	function followers( $page = false )
	{
        ...
        $request = 'http://api.twitter.com/1/statuses/followers.' . $this->type;
        if( $page )
	...
	function getFavorites( $id, $page=false )
	{
		...
		$request = 'http://api.twitter.com/1/favorites.' . $this->type . $qs; 
		return $this->objectify( $this->process($request) );
	}
	...
	function makeFavorite( $id )
	{
		...
		$request = 'http://api.twitter.com/1/favorites/create/' . $id . '.' . $this->type;
		return $this->objectify( $this->process($request) );	
	}
	...
	function removeFavorite( $id )
	{
		...
		$request = 'http://api.twitter.com/1/favorites/destroy/' . $id . '.' . $this->type;
		return $this->objectify( $this->process($request) );	
	}
	...
	function isFriend( $user_a, $user_b )
	{
		...
		$request = 'http://api.twitter.com/1/friendships/exists.' . $this->type . $qs;
		return $this->objectify( $this->process($request) );
	}
	...
	function followUser( $id, $notifications = false )
	{
		...
		$request = 'http://api.twitter.com/1/friendships/create/' . $id . '.' . $this->type;
		if( $notifications )
		    $request .= '?follow=true';
	...
	function leaveUser( $id )
	{
		...
		$request = 'http://api.twitter.com/1/friendships/destroy/' . $id . '.' . $this->type;
		return $this->objectify( $this->process($request) );
	}
	...
	function blockUser( $id )
	{
		...
		$request = 'http://api.twitter.com/1/blocks/create/' . $id . '.' . $this->type;
		return $this->objectify( $this->process($request) );
	}
	...
	function unblockUser()
	{
		...
		$request = 'http://api.twitter.com/1/blocks/destroy/' . $id . '.' . $this->type;
		return $this->objectify( $this->process($request) );
	}
	...
	function socialGraphFollowing( $id = false)
	{
	    ...
	    $request = 'http://api.twitter.com/1/friends/ids';
	    if( $id )
	...
	function socialGraphFollowedBy( $id = false )
	{
	    ...
	    $request = 'http://api.twitter.com/1/followers/ids';
	    if( $id )
	        $request .= '/' . (int) $id . '.' . $this->type;
	...
	function showUser( $id, $email = false, $user_id = false, $screen_name=false )
	{
        ...
        $request = 'http://api.twitter.com/1/users/show/' . $qs . $this->type;
	...
	function directMessages( $since = false, $count = null, $since_id = false, $page = false )
	{
        ...
        $request = 'http://api.twitter.com/1/direct_messages.' . $this->type . implode( '&', $qsparams );
	...
	function sentDirectMessage( $since = false, $since_id = false, $page = false )
	{
        ...
        $request = 'http://api.twitter.com/1/direct_messages/sent.' . $this->type . implode( '&', $qsparams );
        return $this->objectify( $this->process($request) );
	}
	...
	function sendDirectMessage( $user, $text )
	{
        ...
        $request = 'http://api.twitter.com/1/direct_messages/new.' . $this->type;
	...
	function deleteDirectMessage( $id )
	{
	    ...
	    $request = 'http://api.twitter.com/1/direct_messages/destroy/' . (int) $id . '.' . $this->type;
	    return $this->objectify( $this->process( $request ) );
	}
	...
	function updateDevice( $device )
	{
		...
		$qs = '?device=' . $device;
		$request = 'http://api.twitter.com/1/account/update_delivery_device.' . $this->type . $qs;
		return $this->objectify( $this->process( $request ) );
	}
	...
	function updateAvatar( $file )
	{
	    ...
	    $request = 'http://api.twitter.com/1/account/update_profile_image.' . $this->type;
	...
	function updateBackground( $file )
	{
	    ...
	    // Adding @ ensures the POST will be raw multipart data encoded. This MUST be a file, not a URL. Handle it outside of the class.
	    $postdata = array( 'image' => "@$file");
	    $request = 'http://api.twitter.com/1/account/update_profile_background_image.' . $this->type;
	    return $this->objectify( $this->process( $request, $postdata ) );
	}
	...
	function updateProfile( $fields = array() )
	{
	    ...
	    $request = 'http://api.twitter.com/1/account/update_profile.' . $this->type;
	    return $this->objectify( $this->process( $request, $postdata ) );
	}
	...
	function updateColors( $colors = array() )
	{
		...
		$request = 'http://api.twitter.com/1/account/update_profile_colors.' . $this->type;
	    return $this->objectify( $this->process( $request, $postdata ) );
	}
	...
	function ratelimit()
	{
	    if( !in_array( $this->type, array( 'xml','json' ) ) )
	        return false;
		$request = 'http://api.twitter.com/1/account/rate_limit_status.' . $this->type;
	...
	function twitterAvailable()
	{
		...
		$request = 'http://api.twitter.com/1/help/test.' . $this->type;
